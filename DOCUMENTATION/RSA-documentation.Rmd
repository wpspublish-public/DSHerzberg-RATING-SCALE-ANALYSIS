...
#### Commented Snippets
Load packages for file path specification (`here`), data wrangling (`tidyverse`) and pscyhometric data simulation and analysis (`psych`). `base::set.seed()` ensures repeatable results from sampling and modeling functions that use a random-number generator.
...
```{r create-data-rsa, echo = 62, eval = F}
```
We use `readr::write_csv()` to write output to a flat file (.csv). In this instance, the arguments are:

1. `data_input_sim`: data object to be written
2. `here("INPUT-FILES/data-input-sim.csv")`: file path (specified using `here::here()`, which allows you to write the file path out of the R project folder without a leading `/`)
3. `na = ""`: values that are `NA` in R will be written to the .csv as empty cells.

```{r create-data-rsa, echo = 64:66, eval = F}
```
...
```{r create-data-rsa, echo = 67:70, eval = F}
```
