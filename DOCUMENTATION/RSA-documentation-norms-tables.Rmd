...
```
  ...
  CPS1_nt    48    NA
```
Note that the group of rows for `NT = 40`, which previously included eight rows, now consists of only two: those containing the values of `raw` for the lower and upper bounds of the required raw score range `10-17`. All other rows in the example are single rows for one-to-one T-to-raw mappings.
...
As an argument to `summarize()`, we pass the expression `raw = str_c(raw, collapse = '--')`, which returns the required values for both types of T-to-raw mappings. Note that the RHS is wrapped in `str_c`, meaning that numbers will be represented as strings in the new `raw` column. One-to-one mappings are represented in the current object as single rows, so the summarizing expression simply returns the "old" value of `raw` in that row. One-to-many mappings are now represented by two-row groups, with the rows containing the lower and upper bounds of the required raw-score ranges. For these two-row groups, `str_c()` joins the two "old" values of raw, collapsing them into a single string with the separator `--`. Here we use two dashes, as opposed to a single dash, to avoid formatting errors when the output is saved to .csv.

Because the mapping of T-to-raw is one-to-many, there are inevitably possible values of T that are not mapped onto any raw score value. In the rows shown above, the first eight values of raw `10-17` map onto `NT = 40`. The next value of raw `18` maps onto `NT = 42`, leaving `NT = 41` "unmapped" (i.e., `raw` is `NA` for `NT = 41`). To format the `raw` column properly for final output, we use `mutate(across(case_when()))` to recode `NA` to `-` in the `raw` column. We then use arrange to sort the table by `scale`, and descending by `NT`.
...
```
    ...
    75    -       -       27      -       27      -       
```
We can repair inaccurate column names with `rename_with()` which enables the application of `stringr` functions to a batch of column names (here replacing `"_nt"` with `"_raw"`, because the rightward columns contain raw scores). We use `filter(!is.na(T_score))` to drop extraneous rows that are missing T-scores. Finally, we `write_csv()` the finished output to .csv.
