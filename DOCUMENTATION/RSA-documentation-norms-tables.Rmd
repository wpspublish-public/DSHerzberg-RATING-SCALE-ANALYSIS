...
```
  ...
  CPS1_nt    42    18
```
In the final output for the print-format lookup table, we need to represent the raw-score values in a one-to-many mapping as a range of raw scores (e.g., in the rows shown above, as `10-17` to capture the mapping for `NT = 40`). To execute this transformation, we first re-group the data object hierarchically (`group_by(scale, NT)`), so that ranges of `NT` can be assembled for each `scale`.
