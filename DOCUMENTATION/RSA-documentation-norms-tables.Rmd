...
```
We write _robust_ templates that can be adapted to different projects with minimal copying-and-pasting of text. We facilitate this by initializing _tokens_ as vectors. For example, `item_prefix <- "cp"` defines a vector that holds a prefix used in the names of all columns that contain item reponses. We can complete the names of these columns by concatenating the token `item_prefix` with a vector holding a numerical series (e.g., `"001, 002, 003"`). 

Tokens function as placeholders or stand-ins for values that are used repeatedly in the template. When tokens appear in the code, their values are combined with static text elements to create column names, file names, file paths, form names, score ranges, or any other data characteristic that varies from project to project. For example, the code block below includes tokens for reading a specific input file from a specific remote URL. 

In a robust template, the _project-specific_ elements (e.g., remote file paths, scale name prefixes, etc.) are segregated at the front of the script, where they can be entered efficiently as tokens. The remainder of the script consists only of _project-general_ code (e.g., application of normalizing models, generation of raw-to-T lookup tables, etc.); in other words, any functions or operations that will be implemented in the same way for all projects.
