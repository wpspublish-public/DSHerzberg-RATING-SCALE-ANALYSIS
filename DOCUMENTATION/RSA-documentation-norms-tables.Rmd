...
These tokenized elements include names of variables, files, test forms, raters, etc., as well as labels for age- and score ranges, score item counts, score numerical bounds, and so forth. By their vary nature, these elements differ between, say, an autism rating scale project and an ADHD rating scale project. 
...
In R, we can initialize tokens as vectors. In the next code snippet, for example, `item_prefix <- "cp"` defines a vector that holds a prefix used in the names of all columns containing item responses. The snippet also includes tokens for reading a specific input file from a specific remote URL, among other project-specific elements.
```{r norms-load, echo = 5:16, eval = F}
```
Many analytic procedures require the specification of an entire set of project-specific elements, such as all of the item names (for example). With project-specific tokens, we can use string functions to create sets of elements. For instance, we can assemble a vector containing all of the item names by concatenating the token `item_prefix` with a numerical series (e.g., `c("001", "002", "003")`). 

Often, the best name for the resulting set of elements is itself a concatenated string. To use a concantenated string as the name of an object in R, we cannot rely on the conventional assignment operator `<-`. Instead, we use `base::assign()` to initialize these concatenated names. `assign()` takes two arguments, the desired name (concatenated with `stringr::str_c()`), and the code snippet that defines the object to be named.
...
```
###### COMMENTED SNIPPETS
The `bestNormalize` package provides tools for determining the optimal normalization model and applying that model to a raw score distribution. For the latter step, the `bestNormalize()` function requires as input the raw score distribution formatted as a numerical vector.
