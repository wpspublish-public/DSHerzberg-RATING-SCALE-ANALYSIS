...
In addition to the one-to-many mapping for `NT = 40`, the rows above include a one-to-one mappping for `NT = 42` (to `raw = 18`). In the next code snippet, we use `dplyr::filter()` to collapse the table vertically and retain rows according to the following rules:
