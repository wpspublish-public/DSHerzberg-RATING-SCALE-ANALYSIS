...
```
Recall that the structure of the basic format lookup table features all possible raw score values in the left-most column. We use `group_by()` and `summarize()` to begin assembling this column, passing concatenated score names as arguments to both of these functions. 
...
In the code below, the call of `group_by()` groups the data by raw score. At this point, the data object includes all rows in the original input file, meaning the raw score column has many duplicate values. To create a lookup table, we need the raw score column to contain all possible raw scores, with no duplicates.  

To remove the duplicate raw score rows, we can `summarize()` the data object by T-score, passing the name of the required T-score column as a concatenated string, as in the example elaborated above. We take advantage of the many-to-one relationship of raw-to-T, which means that each raw score corresponds to one and only T-score. We can use the summary function `min()` to return the only the rows that contains the minimum T-score associated with each raw score, which by definition is also the _only_ T-score associated with raw score. After applying `summarize()` the data object contains only a single row for each raw score that appears in the data, with its corresponding T-score contained in the same row. Note that within `summarize()`, we need to use the special NSE operator `:=`, instead of a conventional equals sign, when specifying the summary variable.
