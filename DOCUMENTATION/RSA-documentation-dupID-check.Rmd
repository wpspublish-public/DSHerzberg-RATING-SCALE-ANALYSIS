...
`dplyr::mutate()` creates a `dup` column. We populate that column with `base::duplicated(ID)`, which returns `TRUE` for any row containing an additional instance of an ID value (inluding `NA`) that appeared in a previous row.
...
```
The remainder of the code writes the output report to .csv.
