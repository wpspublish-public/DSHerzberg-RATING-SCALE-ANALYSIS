...
The script also produces histograms for each variable, and combines them into a single .pdf output file. These plots are named with the prefix `freq_demos` and saved in the `OUTPUT-FILES/PLOTS` folder.
...
```{r descriptives-freq-demos, echo = 1:5, eval = F}
```
With `mutate()` and `case_when()`, we can recode specific cells of an existing column to produce a more readable output table. In the current data object, the `var` column names a demographic variable, whose categories are contained in the `cat` column.

#### START HERE GIVE EXAMPLE OF VAR-CAT NESTING
```{r descriptives-freq-demos, echo = 9:18, eval = F}
```
