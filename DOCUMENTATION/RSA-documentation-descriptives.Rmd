...
```
Recall that `aes()` specified that the case count for demographic categories will be represented on the y-axis of the plot. We now use `scale_y_continuous()` to add a layer that optimizes the y-axis numerical scale for our data. The `breaks` argument specifies the numeric positions that will be represented explicitly on the vertical scale. Here we pass an ordered numeric vector returned by `base::seq()`, whose arguments are the lowest value `0`, the highest value (`max(.x$n)`, which returns the highest value in the `n` column of the input data element), and increments of `50` for the middle elements of the vector.
...
The descriptive statistics tables provide sample sizes, means, and SDs for the raw scores from five subscales. These frequency tables are named with the prefix `raw_desc` and saved to `.csv` in the `OUTPUT-FILES/TABLES` folder.
